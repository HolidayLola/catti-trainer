...
CATTI 阅卷理念（必须严格遵守）：实务是错误扣分制，阅卷人的工作是找错，不是找亮点。因此：
1. 信 > 达 > 雅。忠实完整第一：错译、漏译、无中生有是最重的扣分；文采不加分。
2. 稳妥直译优先。贴近原文结构、语义准确的直译是满分答案；偏离原文的自由意译即使通顺，只要与原文语义有出入就按错译扣分。若考生的意译虽可接受但有风险，在 x 中提醒"考场上直译更安全"。
3. 规范性：数字、日期、百分比必须准确；人名/地名/机构名/职务用通行标准译名；${dir === "de2zh" ? "汉语标点规范，不得欧化腔（如滥用长定语、被动）；德语长句应拆译为流畅短句" : "德语语法安全第一——没把握的复杂从句宁可拆成两个语法正确的短句，也不写一个有语法错误的长句；官方文体固定表达（如 表示→erklärte, 强调→betonte, 据统计→Statistiken zufolge, 同比增长→im Vorjahresvergleich um X Prozent steigen）必须用规范译法"}。
评分对标真实 CATTI 阅卷严格度，不送人情分，但每处扣分都要有依据；q 精确引用出错文字，x 用中文讲清错在哪、考点是什么，fix 给正确译法；deductions 按原文顺序。
ref 给出"考场最优译文"：稳妥、忠实、贴近原文结构的直译风格，不追求文学性${dir === "zh2de" ? "，德语必须语法零错误、用词正式规范（对标中国官方媒体德语版的行文）" : ""}。sum 写2-4句总评（主要失分模式+针对性建议）。
strategy 写 1-2 条针对本篇的考场策略提示（如哪里该直译求稳、哪类结构考场上如何处理最保险）。
vocab 挑 3-6 个本篇值得积累的词汇/搭配（de 为德语原形带冠词），优先挑考试常考的固定话术。
只输出 JSON：{"score":数字,"max":25,"deductions":[{"q":"","t":"","p":-1,"x":"","fix":""}],"ref":"","sum":"","strategy":"","vocab":[{"de":"","zh":"","note":""}]}
...
function renderFeedback(card, fb, part) {
  ...
  if (fb.strategy) card.appendChild(el("div", "sum", "<b>考场策略</b>　" + esc(fb.strategy)));
